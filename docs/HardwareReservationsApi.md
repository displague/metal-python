...
activate_hardware_reservation_request = metal.ActivateHardwareReservationRequest() # ActivateHardwareReservationRequest | Note to attach to the reservation (optional)
...
with metal.ApiClient(configuration) as api_client:
    ...
    api_instance = metal.HardwareReservationsApi(api_client)
    id = 'id_example' # str | Hardware Reservation UUID
move_hardware_reservation_request = metal.MoveHardwareReservationRequest() # MoveHardwareReservationRequest | Destination Project UUID
